import { askPetAi } from '../server/askAiService.js';

/**
 * Serverless API handler for Ask PetLife AI Q&A endpoint.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { pet, healthEvents, question } = body;
    ...
    if (!question || typeof question !== 'string' || question.trim() === '') {
      return res.status(400).json({ error: "Invalid payload: 'question' must be a non-empty string." });
    }

    if (question.trim().length > 500) {
      return res.status(400).json({ error: "Question exceeds maximum length limit of 500 characters." });
    }
    ...
    // Extract server-side API key
    const apiKey = req.env?.GEMINI_API_KEY || process.env.GEMINI_API_KEY;

    // Call server Gemini Q&A service
    const result = await askPetAi({ pet, healthEvents, question: question.trim() }, apiKey);

    return res.status(200).json(result);
  } catch (err) {
    console.error('[API /api/ask-ai Error]:', err.message);
    const statusCode = err.message?.includes('GEMINI_API_KEY') ? 500 : 500;
    return res.status(statusCode).json({
      error: err.message || 'Internal Server Error during AI question processing.'
    });
  }
}
